...
function handleError(error, context = '') {
  ...
  // Generic error
  console.error(chalk.red(`✖ An error occurred${context ? ` during ${context}` : ''}:`));
  ...
  console.error();
  console.error(chalk.yellow('For help, please visit:'));
...
function handleGitHubError(error) {
  console.error(chalk.red('✖ GitHub API Error:'));

  switch (error.status) {
    case 401:
      console.error(chalk.gray('  Authentication failed. Your GitHub token is invalid or expired.'));
      console.error();
      console.error(chalk.yellow('To fix this:'));
      console.error('  1. Generate a new token at:', chalk.cyan('https://github.com/settings/tokens'));
      console.error('  2. Ensure the token has', chalk.bold('repo'), 'scope');
      console.error('  3. Run:', chalk.cyan('pm-kit init --reset-token'));
      break;

    case 403:
      if (error.message && error.message.includes('rate limit')) {
        console.error(chalk.gray('  GitHub API rate limit exceeded.'));
        console.error();
        console.error(chalk.yellow('To fix this:'));
        console.error('  1. Wait for the rate limit to reset');
        console.error('  2. Use an authenticated token for higher limits');
      } else {
        console.error(chalk.gray('  Access forbidden. You may not have permission to access this repository.'));
        console.error();
        console.error(chalk.yellow('To fix this:'));
    ...
    case 404:
      console.error(chalk.gray('  Repository or resource not found.'));
      console.error();
      console.error(chalk.yellow('To fix this:'));
...
function handleFileSystemError(error) {
  console.error(chalk.red('✖ File System Error:'));

  switch (error.code) {
    case 'EACCES':
      console.error(chalk.gray(`  Permission denied: ${error.path}`));
      console.error();
      console.error(chalk.yellow('To fix this:'));
      console.error('  1. Check file/directory permissions');
      console.error('  2. Try running with appropriate permissions');
      console.error('  3. On Unix/Linux:', chalk.cyan('chmod -R u+w <directory>'));
      break;

    case 'ENOENT':
      console.error(chalk.gray(`  File or directory not found: ${error.path}`));
      console.error();
      console.error(chalk.yellow('To fix this:'));
    ...
    case 'EEXIST':
      console.error(chalk.gray(`  File or directory already exists: ${error.path}`));
      console.error();
      console.error(chalk.yellow('To fix this:'));
    ...
    case 'ENOSPC':
      console.error(chalk.gray('  No space left on device'));
      console.error();
      console.error(chalk.yellow('To fix this:'));
...
function handleNetworkError(error) {
  ...
  switch (error.code) {
    case 'ECONNREFUSED':
      console.error(chalk.gray('  Connection refused. The server is not responding.'));
      console.error();
      console.error(chalk.yellow('To fix this:'));
    ...
    case 'ETIMEDOUT':
      console.error(chalk.gray('  Connection timed out.'));
      console.error();
      console.error(chalk.yellow('To fix this:'));
      console.error('  1. Check your internet connection');
    ...
    case 'ENOTFOUND':
      console.error(chalk.gray('  Could not resolve hostname. DNS lookup failed.'));
      console.error();
      console.error(chalk.yellow('To fix this:'));
      console.error('  1. Check your internet connection');
...
function getErrorGuidance(code) {
  ...
  return chalk.yellow(guidance[code] || 'For help, visit: https://github.com/your-org/claude-kit-pm/issues');
}
