...
function displayResults(results) {
  ...
  for (const category of categories) {
    ...
    for (const check of category.checks) {
      ...
      const requiredLabel = check.required === false ? chalk.gray('(optional)') : '';

      console.log(
        `${icon}  ${check.name} ${requiredLabel}: ${status} - ${chalk.gray(check.message)}`
      );
    }
  ...
  if (totalIssues === 0 && totalWarnings === 0) {
    ...
    logger.newline();
    logger.log(chalk.yellow('To fix issues automatically, run:'), chalk.cyan('pm-kit doctor --fix'));
  }
...
async function performAutoFix(results) {
  ...
  if (fixCount > 0) {
    ...
    logger.log('Run', chalk.cyan('pm-kit doctor'), 'again to verify');
  } else {
    logger.info('No issues could be auto-fixed');
    logger.log('You may need to run', chalk.cyan('pm-kit init'), 'to reinstall');
  }
}
