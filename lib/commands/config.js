...
async function listConfig() {
  ...
  logger.log(chalk.bold('Global Configuration') + chalk.gray(` (${constants.GLOBAL_CONFIG_FILE})`));
  logger.newline();

  if (Object.keys(globalConfig).length === 0) {
    logger.log(chalk.gray('  No global configuration found'));
  } else {
  ...
  logger.log(chalk.bold('Local Configuration') + chalk.gray(` (${constants.LOCAL_CONFIG_FILE})`));
  logger.newline();

  if (Object.keys(localConfig).length === 0) {
    logger.log(chalk.gray('  No local configuration found'));
  } else {
...
async function resetConfig() {
  ...
  logger.log('Run', chalk.cyan('pm-kit init'), 'to set up again');
  logger.newline();
}
