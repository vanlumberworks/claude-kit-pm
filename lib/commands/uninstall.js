...
async function uninstallCommand(options = {}) {
  try {
    ...
    if (installations.length === 0) {
      ...
      logger.log(pc.gray(`  • Local: ${process.cwd()}/.claude`));
      logger.log(pc.gray(`  • Global: ${path.join(os.homedir(), '.claude')}`));
      return;
    }
    ...
    for (const install of installations) {
      ...
      logger.log(`  ${pc.cyan(install.type)}: ${install.path}${pc.gray(sizeInfo)}`);

      if (install.metadata) {
        logger.log(pc.gray(`    Version: ${install.metadata.version || 'unknown'}`));
        logger.log(pc.gray(`    Installed: ${install.metadata.installedAt || 'unknown'}`));
      }
    }
    ...
    if (!options.yes) {
      ...
      for (const install of installations) {
        logger.log(pc.red(`  • ${install.path}`));
      }
      logger.newline();
    ...
    // Summary using @clack/prompts
    note(
      `Removed ${installations.length} installation(s)\n\n` +
      `${pc.bold('What was removed:')}\n` +
      `  • .claude/ directories\n` +
      `  • CLAUDE.md files\n` +
      `  • .mcp.json configurations\n` +
      `  • .pm-kit/ local configs\n\n` +
      `${pc.dim('To reinstall:')} ${pc.cyan('pm-kit init')}`,
      pc.green('✔ Uninstall Complete')
    );

    outro('PM Kit has been removed');
