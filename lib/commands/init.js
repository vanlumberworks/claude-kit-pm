...
async function initCommand(options = {}) {
  try {
    ...
    logger.box(
      ...
      `PM Kit has been installed ${isGlobal ? 'globally' : 'successfully'}.\n\n` +
      `${chalk.bold('Installation location:')}\n` +
      `  ${chalk.cyan(installDir)}\n\n` +
    ...
    logger.newline();
    logger.info('Installation completed in ' + installDir);
...
async function handleFreshInstall(installDir, isGlobal) {
  ...
  logger.log(chalk.red(`This will permanently delete: ${claudePath}`));
  logger.newline();
