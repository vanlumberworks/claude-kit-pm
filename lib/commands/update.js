...
const { confirmAction, note, outro } = require('../utils/prompts');
...
async function updateCommand(options = {}) {
  try {
    ...
    if (changes.length > 0 && !options.force) {
      ...
      if (changes.length > 5) {
        logger.log(pc.gray(`  ... and ${changes.length - 5} more`));
      }
      logger.newline();
    ...
    note(
      `${pc.dim('Version:')} ${versions.current} → ${pc.green(versions.latest)}\n\n` +
      `${pc.bold('What changed:')}\n` +
      `${formatChangelog(versions.changelog)}\n\n` +
      `${pc.bold('Next steps:')}\n` +
      `  1. Run ${pc.cyan('pm-kit doctor')} to verify\n` +
      `  2. Review ${pc.cyan('CLAUDE.md')} for new features`,
      pc.green('✔ Update Complete!')
    );

    outro('PM Kit updated successfully');
...
function displayUpdateInfo(versions) {
  logger.newline();
  logger.table({
    'Current Version': pc.yellow(versions.current),
    'Latest Version': pc.green(versions.latest),
    'Published': new Date(versions.publishedAt).toLocaleDateString(),
    'Status': versions.status === 'outdated' ? pc.yellow('Update available') : pc.green('Up to date')
  });
  logger.newline();

  if (versions.changelog) {
    logger.log(pc.bold('Release Notes:'));
    logger.log(pc.gray(versions.changelog));
    logger.newline();
  }
}
